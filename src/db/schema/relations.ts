...
export const movieRelations = relations(movie, ({ one, many }) => ({
  ...
  credits: one(movieCredits, {
    fields: [movie.id],
    references: [movieCredits.id],
  }),
...
export const recommendedMovieRelations = relations(
  recommendedMovie,
  ({ one }) => ({
...
export const movieToShortlistRelations = relations(
  movieToShortlist,
  ({ one }) => ({
    movie: one(movie, {
