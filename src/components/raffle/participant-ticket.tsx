...
import { Film, Plus, Star, XCircle } from 'lucide-react'
...
function TicketMovieRow({
  ...
  return (
    <div
      className="flex items-center gap-3 p-2 rounded-md bg-[var(--ticket-movie-bg)]"
...
function getStampClass(participating: boolean, isReady: boolean) {
  const baseClass =
    'absolute top-3 right-3 px-2.5 py-1 border-2 border-transparent rounded-sm text-[10px] font-bold tracking-[2px] uppercase font-[var(--font-cinema)] transition-all duration-200 cursor-pointer hover:scale-110 hover:shadow-md'
  if (!participating)
    return `${baseClass} bg-[var(--ticket-empty-bg)] border-[var(--ticket-empty-border)] hover:bg-[var(--ticket-empty-bg)]/80`
  if (isReady)
    return `${baseClass} text-[var(--ticket-stamp-ready)] border-[var(--ticket-stamp-ready)] bg-[var(--ticket-stub-ready-bg-start)] shadow-[0_0_0_1px_oklch(0.75_0.15_85/0.3),0_0_12px_oklch(0.6_0.15_85/0.2)_inset] drop-shadow-[0_0_8px_oklch(0.6_0.15_85/0.4)] hover:shadow-[0_0_0_1px_oklch(0.75_0.15_85/0.5),0_0_16px_oklch(0.6_0.15_85/0.3)_inset]`
  return `${baseClass} text-[var(--ticket-stamp-not-ready)] border-[var(--ticket-stamp-not-ready)] bg-[var(--ticket-movie-bg)] hover:bg-[var(--ticket-movie-bg)]/80`
}
...
export function ParticipantTicket({
  ...
  return (
    <div className="relative">
      ...
      <div
        className={`ticket-card relative rounded overflow-hidden transition-transform duration-[250ms] ease-[cubic-bezier(0.16,1,0.3,1)] cursor-pointer bg-gradient-to-br from-[var(--ticket-bg-start)] to-[var(--ticket-bg-end)] ${
          participating
            ? 'hover:-translate-y-1.5 hover:rotate-[-0.3deg] hover:shadow-[0_12px_24px_rgba(0,0,0,0.25),0_24px_48px_rgba(0,0,0,0.15)]'
            : 'opacity-60'
        }`}
