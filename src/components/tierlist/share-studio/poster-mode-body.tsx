...
export function PosterModeBody({
  ...
  const posterDimensions = useMemo(() => {
    switch (settings.posterSize) {
      case 'sm': return { width: 70, gap: 8 }
      case 'lg': return { width: 110, gap: 12 }
      default: return { width: 90, gap: 10 }
    }
  }, [settings.posterSize])

  const labelSize = settings.posterSize === 'lg' ? 'lg' : settings.posterSize === 'sm' ? 'sm' : 'md'
  ...
  const labelColumnWidth = useMemo(() => {
    ...
    for (const tier of rankedTiers) {
      ...
      const estimated = estimateLabelWidth(label, labelSize, settings.tierLabelShape)
      max = Math.max(max, estimated)
    }
    return Math.min(max, 180)
  }, [rankedTiers, settings.showTierLabels, settings.tierLabelShape, labelSize, getLabel])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      {rankedTiers.map((tier) => {
        ...
        return (
          <div key={tier.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: posterDimensions.gap, flex: 1, alignContent: 'flex-start' }}>
              {tier.movies.map((m) => (
                <PosterTile key={m.id} movie={m} width={posterDimensions.width} theme={theme} />
              ))}
            </div>
          </div>
        )
